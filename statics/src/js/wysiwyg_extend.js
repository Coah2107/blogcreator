    ...
    Wysiwyg.include({
        ...
        _onBlogCreatorImageClick: function () {
            ...
            this._rpc({
                model: 'ir.attachment',
                method: 'search_read',
                domain: [['res_model', '=', 'blogcreator.note'], ['mimetype', 'ilike', 'image']],
                ...
                if (!attachments.length) {
                    $content.append($('<div class="col-12 text-center"><p>Không có hình ảnh nào trong thư viện</p></div>'));
                }
                
                // Hiển thị các hình ảnh
                _.each(attachments, function (attachment) {
                    var $col = $('<div class="col-3 mb-3"></div>');
                    var $img = $('<img class="img-thumbnail img-fluid cursor-pointer"/>').attr('src', 'data:' + attachment.mimetype + ';base64,' + attachment.datas);
                    var $name = $('<p class="text-center mb-0"></p>').text(attachment.name);
                    ...
                    $img.on('click', function () {
                        ...
                        self.odooEditor.execCommand('insertHTML', '<img src="' + imgUrl + '" alt="' + attachment.name + '"/>');
                        dialog.close();
                    });
                });
